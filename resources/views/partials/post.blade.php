    ...
    <div class="w-full lg:w-10/12 2xl:w-8/12 mx-auto border-solid border-1 border-border flex flex-col rounded-xl shadow-xl">
        ...
        <div class="w-full border-b-1 border-solid border-border px-4 py-4 flex flex-row justify-start items-start">
            ...
            <div class="pr-5 w-full md:w-10/12">
                <form method="post" action="{{ route('dashboard') }}" class="w-full" autocomplete="off">
                    @csrf
                    <div class="w-full">
                        <label for="post"></label>
                        <textarea name="post" id="post" rows="4"  class="w-full focus:outline-none text-p  @error('post') border-solid border-1 border-queen-blue @enderror"
                                  maxlength="150" placeholder="O čemu razmišljate?"></textarea>

                        @error('post')
                            <div class="text-fiery-red mt-2 text-xs">
                                {{ $message }}
                            </div>
                        @enderror
                    </div>
                    <div class="w-full flex justify-between items-center mt-5">
                        ...
                        <input type="submit" class="w-1/3 lg:w-1/4 px-4 py-2 bg-queen-blue font-normal text-sm tracking-wide text-white rounded-full hover:bg-fiery-red focus:bg-fiery-red" value="Objavi">
                    </div>
                </form>
            </div>
        </div>
